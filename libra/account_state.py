...
class AccountState(Struct):
    ...
    @classmethod
    def from_blob_or_default(cls, blob: Optional[bytes]):
        if blob is None:
            return AccountState({})
        else:
    ...
    def get_resource(self, path=None, T=AccountResource):
        if path is None:
    ...
    def get_libra_block_resource(self):
