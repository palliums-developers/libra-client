...
class MultiEd25519PrivateKey(Struct):
    ...
    def fromEd25519PrivateKey(ed_private_key: Ed25519PrivateKey):
...
class MultiEd25519PublicKey(Struct):
    ...
    @classmethod
    def fromEd25519PublicKey(cls, ed_public_key: Ed25519PublicKey):
        return MultiEd25519PublicKey(
    ...
    @classmethod
    def fromMultiEd25519PrivateKey(cls, private_key: MultiEd25519PrivateKey):
...
class MultiEd25519Signature(Struct):
    ...
    @classmethod
    def fromEd25519Signature(cls, ed_signature: Ed25519Signature):
...
def bitmap_set_bit(bitmap: List[Uint8], index):
    bucket = index // 8
...
def bitmap_get_bit(bitmap: List[Uint8], index) -> bool:
    bucket = index // 8
    # It's always invoked with index < 32, thus there is no need to check range.
    bucket_pos = index - (bucket * 8)
...
def bitmap_last_set_bit(bitmap: List[Uint8]):
    bitvec = flatten([bin(x)[2:].rjust(8, '0') for x in bitmap])
    ret = None
    for (i, bit) in enumerate(bitvec):
        if bit == '1':
